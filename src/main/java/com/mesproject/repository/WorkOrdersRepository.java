...
public interface WorkOrdersRepository extends JpaRepository<WorkOrders, Long> {
    ...
    @Query("SELECT NEW com.mesproject.dto.WorkOrdersDto2(w.workOrderId, p.productName, w.processType, w.worker, w.scheduledDate, w.duration, w.start, w.end, w.workOrdersStatus) " +
            "FROM WorkOrders w " +
...
//            "      (:searchType = '작업명' AND w.processType LIKE %:searchValue%) OR " +
